...
public class Monitor extends Thread {
	...
	//Practica 8
	private boolean suspende = true;
	public int numeroSuspendido = 0;
	private JLabel etiquetaSuspendidos;
	...
	public Monitor(Vector<Proceso> listaNuevos, int q) {
	    ...
	    etiquetaSuspendidos = new JLabel("Suspendidos :"+numeroSuspendido);
	    ventanaMonitor.add(etiquetaSuspendidos);
	    etiquetaSuspendidos.setBounds(1000, 500, 150, 60);
	...
	//Parecido a liberar pero se manda a escribir en un archivo
	public void mandaSuspender(){
		String linea = null;
		try {
			
			BufferedReader lector = null;
			BufferedWriter escritorNuevo = null;
			//Cerramos el lector
			if(numeroSuspendido == 0){
				escritorNuevo = new BufferedWriter(new FileWriter(new File("Suspendidos.txt")));
				//Escribimos el contenido del proceso bloqueado
				escritorNuevo.write(colaBloqueados.firstElement().getOperacion()+"|");
				escritorNuevo.write(colaBloqueados.firstElement().getTiempo()+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tiempoRestante+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tiempoLlegada+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tiempoFinalizacion+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tiempoRetorno+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tiempoRespuesta+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tiempoEspera+"|");
				escritorNuevo.write(5+"|");
				escritorNuevo.write(colaBloqueados.firstElement().segundaVez+"|");
				escritorNuevo.write(colaBloqueados.firstElement().primeraVez+"|");
				escritorNuevo.write(colaBloqueados.firstElement().ejecutando+"|");
				escritorNuevo.write(colaBloqueados.firstElement().getDato1()+"|");
				escritorNuevo.write(colaBloqueados.firstElement().getDato2()+"|");
				escritorNuevo.write(colaBloqueados.firstElement().getTiempo()+"|");
				escritorNuevo.write(colaBloqueados.firstElement().getID()+"|");
				escritorNuevo.write(colaBloqueados.firstElement().tamaño+"");
				escritorNuevo.newLine();
				...
				md3.removeRow(0);
				lista3.setModel(md3);
				colaBloqueados.remove(colaBloqueados.firstElement());
				etiquetaSuspendidos.setText("Suspendido: "+(numeroSuspendido + 1));
				
				escritorNuevo.close();
				numeroSuspendido++;
			}
			else{ // Sobreescribimos el nuevo con el viejo
				lector = new BufferedReader( new FileReader ( new File ("Suspendidos.txt")));
				escritorNuevo = new BufferedWriter(new FileWriter(new File("Nuevo.txt")));
				while((linea = lector.readLine()) != null){
					escritorNuevo.write(linea); 
					escritorNuevo.newLine();
				}
				...
				lector.close();
				//Renombrambos
				File originalE = new File("Suspendidos.txt");
				File nuevoE = new File("Nuevo.txt");
				originalE.delete();
	...
	//Primero verifica que haya espacio, si lo hay lo colocamos
	public void liberaSuspendido(){
		Proceso sB  = new Proceso();		
		String linea = null;
		...
		try {
			BufferedReader lector = new BufferedReader( new FileReader ( new File ("Suspendidos.txt")));
			BufferedWriter escritorNuevo = new BufferedWriter(new FileWriter(new File("Nuevo.txt")));

			linea = lector.readLine();
			separador = new StringTokenizer(linea, "|");
			sB.setOperacion(separador.nextToken().charAt(0));
			sB.setTiempo(Integer.parseInt(separador.nextToken()));
			sB.tiempoRestante = Integer.parseInt(separador.nextToken());
			sB.tiempoLlegada = Integer.parseInt(separador.nextToken());
			sB.tiempoFinalizacion = Integer.parseInt(separador.nextToken());
			sB.tiempoRetorno = Integer.parseInt(separador.nextToken());
			sB.tiempoRespuesta = Integer.parseInt(separador.nextToken());
			sB.tiempoEspera = Integer.parseInt(separador.nextToken());
			sB.tiempoBloqueado = Integer.parseInt(separador.nextToken());
			sB.segundaVez = Integer.parseInt(separador.nextToken());
			sB.primeraVez = Boolean.parseBoolean(separador.nextToken());
			sB.ejecutando = Boolean.parseBoolean(separador.nextToken());
			sB.setDato1(Integer.parseInt(separador.nextToken()));
			sB.setDato2(Integer.parseInt(separador.nextToken()));
			sB.setNumeroID(separador.nextToken());
			sB.tamaño = Integer.parseInt(separador.nextToken());
			

			marcosOcupa = sB.tamaño / 4;
			//Van a sobrar agregagmos un marco Mas
			if(sB.tamaño % 4 != 0)
				marcosOcupa++;
			encontro = false;
			...
			if(finMarco - inicioMarco == marcosOcupa){
				encontro = true;
				paginasUsar = sB.tamaño;
				puedeEntrar = true;
				
				for(int i = inicioMarco + 2; i <= finMarco + 1 ; i++){
					if(paginasUsar >= 4){
						paginas[i] = "4 + Bloqueado";
						paginasUsar -= 4;
					}
					else if(paginasUsar <= 0){
						break;
						...
						paginas[i] = paginasUsar + " + Bloqueado";
						break;
					}
			...
			if(puedeEntrar){
				fila[0] = sB.getID();
				fila[1] = Integer.toString(sB.tiempoBloqueado);
				fila[2] = Integer.toString(sB.tamaño);

				
				md3.addRow(fila);
				colaBloqueados.add(sB);
				
				//Sobreescribimos el contenido restante para un nuevo archivo
			...
			else{//Caso de que no encontro espacio no lo mandamos y seguimos con el mismo archivo.
				escritorNuevo.write(linea);
				escritorNuevo.newLine();
				while((linea = lector.readLine()) != null){
					escritorNuevo.write(linea);
	...
	public void run(){
		...
		//Iteracion donde cuando terminen las tres colas o no tengamos procesos suspendidos saldra del programa saldra el programa
		while(colaListos.size() > 0 || colaNuevos.size() > 0 || colaBloqueados.size() > 0 || numeroSuspendido > 0){
			contadorTrans = 0;
			quantumProceso = 0;
			if(colaListos.size() == 0)
				correBloqueados();
			
			//Corremos por nosotros mismos el reloj
			if(colaListos.size() == 0 && colaNuevos.size() == 0 || colaBloqueados.size() == 0){
				relojGlobal.setText("Reloj Global: "+(contGlobal));
				try {
					ventanaMonitor.requestFocus();
					Thread.sleep(1000);
