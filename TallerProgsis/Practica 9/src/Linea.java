...
public class Linea {
	...
	private void calculaDirectivasConstantes(){
		...
		if(codop.equalsIgnoreCase("DB") || codop.equalsIgnoreCase("DC.B") || codop.equalsIgnoreCase("FCB")){
			...
			operandoP = operandoP.substring(1,operandoP.length()-3);
...
//Practica 4
	...
	private void validaDirectivaCaracteres(){
		...
		if(!operando.startsWith("\"") && !operando.startsWith("“")){
			...
			if(!tengoErrores()){
				System.out.println(operando.length() + "hola"+resta) ;
...
// Practica 3
	public void validarOperando(int contador){
...
//Indexado D Indirecto
	...
	//Relativo de 8 bytes
	private void validaRelativo8(int contador){
		int num;
		Convertidor pruebaOperando = new Convertidor();
		if(operando.matches("\\$[0-9A-Fa-f]+") || operando.matches("\\@[0-7]+") ||
		   operando.matches("\\%[0-1]+") || operando.matches("[\\+|\\-]?[0-9]+")){
			...
			if(num >= -128 && num <= 127){
				modoActual = "REL8";
			if(!(num - contador >= -128 && num- contador <=127))
					{
				errorOperando = true;
					}
			}
	...
	private void validaRelativo16(int contador){
		int num;
		Convertidor pruebaOperando = new Convertidor();
		if(operando.matches("\\$[0-9A-Fa-f]+") || operando.matches("\\@[0-7]+") ||
		   operando.matches("\\%[0-1]+") || operando.matches("[\\+|\\-]?[0-9]+")){
			...
			if(num >= -32768 && num <= 65535){
				modoActual = "REL16";
				if(!(num - contador >= -32768 && num- contador <= 32767))
				{
					errorOperando = true;
				}
			}
