...
public class Linea {
	...
	public boolean directivaConstante = false;
	...
	public final int desplazamientoFueraRango8 = 54;
	public final int desplazamientoFueraRango16 = 55;
	public final int noEncontroExtEt = 60;
	public final int noEncontroEtRel8 = 65;
	public final int noEncontroEtRel16 = 66;
	...
	public void calculaCodigoMaquina(Arbol tab, Vector <EtiquetaTabSim> etiquetas, int contador){
		...
		if(!directivaConstante){
			...
			else if(modoActual.equals("IDX"))
				calculaIndexado(tab);
			else if(modoActual.equals("IDX1"))
				calculaIndexado1(tab);
			else if(modoActual.equals("IDX2"))
				calculaIndexado2(tab);
			else if(modoActual.equals("[D,IDX]"))
				calculaIndexadoIndirectoD(tab);
			else if(modoActual.equals("[IDX2]"))
				calculaIndexado2Indirecto(tab);
			else if(modoActual.equals("REL8"))
				calculaRelativo8(tab, etiquetas, contador);
			else if(modoActual.equals("REL16"))
				calculaRelativo16(tab,etiquetas, contador);
		}
		else
			calculaDirectivasConstantes();
	}
	...
	//Practica 6
	private String obtenRegistro(String registro){
		String nuevo =  "";
	...
	private void calculaIndexado(Arbol tab){
		Tabop analiza = null;
		instruccion = tab.buscaDirectiva(codop);
		String registro = "";
		String numero = "";
		...
		//Primero obtenemos el codigo maquina
		codMaq = analiza.getCodigoMaquina();
		//Revisamos los casos de indexado por el que inicia con coma
		switch(separaComa.countTokens()){
		
			case 1:
				registro = separaComa.nextToken();
				//rr0nnnnn son 0 porque por default en binario es 0 al comenzar con coma
				xb = obtenRegistro(registro) + "0" + "00000";
				//convierte Hexa
				xb = Integer.toHexString(Integer.parseInt(xb,2));
				xb = rellenaBytes((byte) 1,xb);
				codMaq += xb;
				break;
			case 2:
				
				numero = separaComa.nextToken();
				registro = separaComa.nextToken();
				//Realizamos el caso pre/post
				if(registro.equalsIgnoreCase("+x") || registro.equalsIgnoreCase("-x") || registro.equalsIgnoreCase("x+") ||
				   ...
				   registro.equalsIgnoreCase("-sp") || registro.equalsIgnoreCase("sp+") || registro.equalsIgnoreCase("sp-")){
					...
					//Obtenemos el numero y le restamos 1
					if(positivo){
						numero = Integer.toBinaryString(c.regresaConversion(numero)-1);
					...
					//Los demas estan en complemento a dos
					else{
						numero = Integer.toBinaryString(~c.regresaConversion(numero) + 1);
					...
					xb = obtenRegistro(registro) + "1" + p + numero;
					xb = Integer.toHexString(Integer.parseInt(xb,2));
					xb = rellenaBytes((byte) 1,xb);
				...
				//Caso acumulador
				else if(numero.equalsIgnoreCase("A") || numero.equalsIgnoreCase("B") || numero.equalsIgnoreCase("D")){
					if(numero.equalsIgnoreCase("A"))
						acumulador = "00";
					else if(numero.equalsIgnoreCase("B"))
						acumulador = "01";
					else
						acumulador = "10";
					
					xb = "111" + obtenRegistro(registro) + 1 + acumulador;
					xb = Integer.toHexString(Integer.parseInt(xb,2));
					xb = rellenaBytes((byte) 1,xb);
				...
				//Caso normal 5 bits
					...
					numero = Integer.toBinaryString(c.regresaConversion(numero));
					if(numero.length() > 4)
					...
					xb = obtenRegistro(registro) + "0" + numero;
					xb = Integer.toHexString(Integer.parseInt(xb,2));
					xb = rellenaBytes((byte) 1,xb);
	...
	private void calculaIndexado1( Arbol tab){
		Tabop analiza = null;
		instruccion = tab.buscaDirectiva(codop);
		String registro = "";
		String numero = "";
		...
		valor = c.regresaConversion(numero);
		if(valor >= 0)
			s = "0";
		else
			s = "1";
		
		xb = "111" + obtenRegistro(registro) + "0" + z + s;
		xb = Integer.toHexString(Integer.parseInt(xb,2));
		xb = rellenaBytes((byte) 1,xb);
		
		codMaq += xb;
		
		//Concatenamos el valor del operando
		numero = Integer.toHexString(valor);
		if(valor >= 0)
			numero = rellenaBytes((byte) (analiza.getBytesCalcular()-1),numero);
		else
			numero = ajustaBytes((byte) (analiza.getBytesCalcular()-1),numero);
		
		codMaq += numero;
		
	}
	
	private void calculaIndexado2( Arbol tab){
		Tabop analiza = null;
		instruccion = tab.buscaDirectiva(codop);
		String registro = "";
		String numero = "";
	...
	//Practica 7
		private void calculaIndexadoIndirectoD(Arbol tab){
			Tabop analiza = null;
			instruccion = tab.buscaDirectiva(codop);
			String registro = "";
			String xb = "";
			...
			separaComa.nextToken();
			registro = separaComa.nextToken();
			registro = registro.substring(0,registro.length()-1);
			
			xb = "111" + obtenRegistro(registro) + "111";
			xb = Integer.toHexString(Integer.parseInt(xb,2));
			xb = rellenaBytes((byte) 1,xb);
		...
		private void calculaIndexado2Indirecto(Arbol tab){
			Tabop analiza = null;
			instruccion = tab.buscaDirectiva(codop);
			String registro = "";
			String numero = "";
			...
			xb = "111" + obtenRegistro(registro) + "011";
			xb = Integer.toHexString(Integer.parseInt(xb,2));
			xb = rellenaBytes((byte) 1,xb);
	...
	//Practica 8
		private void calculaRelativo8(Arbol tab, Vector <EtiquetaTabSim> etiquetas, int contador){
			Tabop analiza = null;
			Convertidor conv = new Convertidor();
			int direccion = 0;
			int valorContSiguiente = 0;
			...
			if(operando.matches("^[A-Za-z][A-Za-z0-9_]*")){
				encontrado = false;
				for(int i= 0, j = etiquetas.size(); i < j ; i++){
					if(etiquetas.elementAt(i).etiqueta.equals(operando)){
						direccion = conv.regresaConversion(etiquetas.elementAt(i).valorContLoc);
						encontrado = true;
						break;
					}
				}
				...
				direccion = conv.regresaConversion(operando);
			...
			//En este punto empezamos con el valor de desplazamiento, el valor del contador siguiente es la  del contador actual mas la suma de
			//todos los bytes
				valorContSiguiente = analiza.getTotalBytes() + contador;
				if(direccion >= valorContSiguiente)
					valorDesplazamiento =  direccion - valorContSiguiente;
				else
					valorDesplazamiento =  ~(valorContSiguiente - direccion) + 1 ;
				//Si el desplazamiento entra en el rango lo usamos para la conversion y si es negativo le hacemos complemento
				if(valorDesplazamiento >= -128 && valorDesplazamiento <= 127){
					codigoOperando = Integer.toHexString(valorDesplazamiento);
					codMaq = analiza.getCodigoMaquina();
					if(analiza.getBytesCalcular() > 0){
		...
		private void calculaRelativo16(Arbol tab, Vector <EtiquetaTabSim> etiquetas, int contador){
			Tabop analiza = null;
			Convertidor conv = new Convertidor();
			int direccion = 0;
			int valorContSiguiente = 0;
			...
			if(!encontrado){
				errorOperando = true;
				numErrorOperando = noEncontroEtRel16;
			...
			//todos los bytes
				...
				if(direccion >= valorContSiguiente)
					...
					valorDesplazamiento =  ~(valorContSiguiente - direccion) + 1 ;
				//Si el desplazamiento entra en el rango lo usamos para la conversion -32768 a 32767.
				if(valorDesplazamiento >= -32768 && valorDesplazamiento <= 32767){
					codigoOperando = Integer.toHexString(valorDesplazamiento);
					codMaq = analiza.getCodigoMaquina();
					if(analiza.getBytesCalcular() > 0){
						if(valorDesplazamiento >= 0)
	...
	//Practica 9
	private void calculaDirectivasConstantes(){
		Convertidor prueba = new Convertidor();
		int num = 0;
		String cadenaFcc = "";
		String operandoP = operando;
		int ultimaComilla = 0;
		int p = 1;
		try {
			operandoP = new String(operandoP.getBytes(),"UTF-8");
		...
		if(codop.equalsIgnoreCase("DB") || codop.equalsIgnoreCase("DC.B") || codop.equalsIgnoreCase("FCB")){
			num = prueba.regresaConversion(operando);
			codMaq = rellenaBytes((byte)1,Integer.toHexString(num));	
		}
		else if(codop.equalsIgnoreCase("DW") || codop.equalsIgnoreCase("DC.W") || codop.equalsIgnoreCase("FDB")){
			num = prueba.regresaConversion(operando);
			codMaq = rellenaBytes((byte)2, Integer.toHexString(num));
		}
		else if(codop.equalsIgnoreCase("FCC")){
			//Quitamos las comillas para probar la cadena
			while(p > 0){
				p = operandoP.indexOf("\"",ultimaComilla + 1);
				if(p > 0)
					ultimaComilla = p;
			}
			operandoP = operandoP.substring(1,ultimaComilla);
			for(int i = 0, j = operandoP.length(); i < j; i++ ){
				if(operandoP.charAt(i) == '\\' && ((i + 1) < j)){
					//Saltamos la diagonal por lo que leemos la diagonal siguiente
					i++;
					num = (int)operandoP.charAt(i);
					cadenaFcc += rellenaBytes((byte)1, Integer.toHexString(num));
				}
				else{
					num = (int) operandoP.charAt(i);
					cadenaFcc += rellenaBytes((byte)1,Integer.toHexString(num));
		...
		if(!operando.startsWith("\"") && !operando.startsWith("“")){
			...
			if(resultado.indexOf("\\", segundaComilla-1) > 0){
				while(i > 0){
					...
					if(resultado.indexOf("\\", i-1) > 0 && i > 0){
						segundaComilla = i;
						if(resultado.charAt(segundaComilla-2) == '\\'){
							resta--;
							...
							//Volvemos a juntar el operando para obtener la cadena original
								...
								resultado = operando.substring(0,segundaComilla+1);
								operando = resultado;
								break;
							}
						}
			...
			if(!tengoErrores()){
				...
				sumaContDirectiva += resta;
			}
		}
	}
	
	public boolean esMemoriaReserva(){
...
// Practica 3
	...
	private void validaIndexado(){
		...
		StringTokenizer separaComa = new StringTokenizer(operando,",");
		if(operando.equalsIgnoreCase("null") && instruccion.modos.elementAt(0).getBytesCalcular() == 0)
			modoActual = "";
		else if(!operando.startsWith("[") && !operando.endsWith("]")){
			switch(separaComa.countTokens()){
		...
		//Error comun de codigo de operacion
			...
			switch(numErrorOperando){
				...
				case endConCodop:
					...
					break;
				case desplazamientoFueraRango8:
					error = "Desplazamiento fuera de rango para relativo de 8" + codop;
					break;
				case desplazamientoFueraRango16:
					error = "Desplazamiento fuera de rango para relativo de 16"+ codop;
					break;
				case noEncontroExtEt:
					error = "El operando llama a una etiqueta inexistente";
					break;
				case noEncontroEtRel8:
					error = "El operando relativo de 8 "+codop + "hace llamado a una etiqueta inexistente";
					break;
				case noEncontroEtRel16:
					error = "El operando relativo de 16 "+ codop+ "hace llamado a una etiqueta inexistente";
					break;
			}
		}
